...
public:
  ...
  void StartClient2 (OnOffHelper client);
  void StartClient3 (ThreeGppHttpServerHelper server);
  /**
...
WifiDlOfdmaExample::Setup (void)
{
  ...
  serverAddress = ApInterface.GetAddress (0);
...
m_sinkApps = clientHelper.Install (m_staNodes);
   m_sinkApps.Stop (Seconds (m_warmup + m_simulationTime + 100));
 // let the server be active for a long time
...
WifiDlOfdmaExample::Run (void)
{
  ...
  // // std::cout<<"sohaib chutiya";
  // AnimationInterface anim("code1.xml");
  // anim.SetMaxPktsTraceFile(50000);
  // float* x=new float[6];
  // float* y=new float[6];
  // for(int i=0;i<6;i++){
  //   x[i]=0;
  ...
  // for(uint32_t i=0;i<m_staNodes.GetN();i++)
  // {
  //   anim.SetConstantPosition(m_staNodes.Get(i),x[i],y[i]);
    
  // }
  // anim.SetConstantPosition(m_apNodes.Get(0),x[5],y[5]);
  ...
  //   {
  //     if(i<=2){
  //     tput = ((m_rxStop[i]) * 8.) / ((m_warmup) * 1e6);}
  //     else
...
//client.SetAttribute ("MaxBytes", UintegerValue (102400));
...
InetSocketAddress dest (m_staInterfaces.GetAddress (m_currentSta), m_port);
  ...
  Simulator::Schedule (MilliSeconds (static_cast<uint64_t> (startTime) + 110) - Simulator::Now (),
                       &WifiDlOfdmaExample::StartClient1, this, client);
}
else if(m_currentSta<=15)
{
  OnOffHelper client (socketType, Ipv4Address::GetAny ());
  client.SetAttribute ("OnTime", StringValue ("ns3::ExponentialRandomVariable[Mean=0.35]"));
  client.SetAttribute ("OffTime", StringValue ("ns3::ExponentialRandomVariable[Mean=0.65]"));
  client.SetAttribute ("DataRate", DataRateValue (DataRate (m_dataRate * 1e6)));
  ...
  Simulator::Schedule (MilliSeconds (static_cast<uint64_t> (startTime) + 110) - Simulator::Now (),
                       &WifiDlOfdmaExample::StartClient2, this, client);

}
else 
{
  
  ThreeGppHttpServerHelper server (serverAddress );
  uint64_t startTime = std::ceil (Simulator::Now ().ToDouble (Time::MS) / offInterval) * offInterval;

  Simulator::Schedule (MilliSeconds (static_cast<uint64_t> (startTime) + 110) - Simulator::Now (),
                       &WifiDlOfdmaExample::StartClient3, this, server);
}
...
void
WifiDlOfdmaExample::StartClient1 (BulkSendHelper client)
{
  NS_LOG_FUNCTION (this << m_currentSta);
...
void
WifiDlOfdmaExample::StartClient2 (OnOffHelper client)
{
  NS_LOG_FUNCTION (this << m_currentSta);
...
void
WifiDlOfdmaExample::StartClient3 (ThreeGppHttpServerHelper server)
{
  NS_LOG_FUNCTION (this << m_currentSta);

  m_clientApps.Add (server.Install (m_apNodes));
  // let clients be active for a long time
  ...
  for (uint32_t i = 0; i < m_staNodes.GetN (); i++)
    {
      ...
      if(i<=10){
     //clientApp->SetAttribute ("MaxBytes", UintegerValue (102400));
//
      }
      else if(i<=15)
      {
      ...
      clientApp->SetAttribute("SendSize",UintegerValue (m_payloadSize));

      }
else{
  Ptr<ThreeGppHttpServer> httpServer = m_clientApps.Get (i)->GetObject<ThreeGppHttpServer> ();
   PointerValue varPtr;
  ...
  Simulator::Schedule (Seconds (m_warmup+1), &WifiDlOfdmaExample::StartStatistics, this);
}
